...
public class Main {
	...
	private static void allocateMemory(long bytes) {
		try {
			Logging.log(String.format(
					"Attempting to allocate %s bytes of memory.",
					formatNumber(bytes)));
			...
			if (bytes > MAX_ARRAY_SIZE) {
				...
				// Two-dimensional array containing an array of MAX_ARRAY_SIZE
				// per chunk.
			...
			printMemoryUsage();
			Logging.log(String.format(
					"SUCCESSFULLY allocated %s bytes of memory.",
					formatNumber(bytes)));
		} catch (OutOfMemoryError error) {
			Logging.log(String.format("FAILED to allocate %s bytes of memory.",
					formatNumber(bytes)));
	...
	/**
	 * Gets the number of maximum-sized arrays that can be created within the
	 * passed number of bytes.
	 *
	 * @param bytes
	...
	private static void printMemoryUsage() {
		try {
			...
			Logging.log(String.format(format, "COMMITTED", "INIT", "USED",
					"MAX"));
